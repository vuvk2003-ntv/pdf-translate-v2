...
class GoogleTranslateEngine(
    ...
    companion object {
        const val MAXIMUM_SEGMENT_CHARACTERS = 5000
    }
    ...
    fun translate(rawText: String, ignoreCache: Boolean = false): String {
        ...
        if (encodedText.length > MAXIMUM_SEGMENT_CHARACTERS) {
            throw IOException(
                "Segment of ${encodedText.length} characters exceeds the " +
                    "$MAXIMUM_SEGMENT_CHARACTERS the translation service accepts"
            )
        }
