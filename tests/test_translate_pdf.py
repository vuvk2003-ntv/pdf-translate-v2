...
class FailureReportingTests(unittest.TestCase):
    ...
    def test_technical_corruption_is_not_reported_as_a_network_failure(self):
        lines = translate_pdf._describe_failures(Counter({"TechnicalInvariantError": 2}))
        self.assertEqual(len(lines), 1)
        self.assertIn("protected technical data", lines[0])


class CoverageAccountingTests(unittest.TestCase):
    def test_all_extracted_segments_are_accounted_for(self):
        validate_segment_accounting(12, translated=7, preserved=3, unresolved=2)

    def test_a_dropped_segment_fails_accounting(self):
        with self.assertRaisesRegex(RuntimeError, "every extracted segment"):
            validate_segment_accounting(12, translated=7, preserved=3, unresolved=1)
...
class TranslatePdfTests(unittest.TestCase):
    ...
    def test_primary_technical_source_languages_are_not_forced_to_english(self):
        for language in ("auto", "ko", "en", "zh-CN", "zh-TW"):
            with self.subTest(language=language):
                self.assertEqual(translate_pdf._source_language(language), language)
    ...
    def test_handoff_output_cannot_overwrite_the_source_pdf(self):
        with mock.patch.object(translate_pdf, "_require_core"):
            with self.assertRaisesRegex(translate_pdf.TranslationError, "protected source"):
                translate_pdf.translate_pdf(
                    self.source,
                    None,
                    engine="handoff",
                    emit_segments=self.source,
                )
    ...
    def test_confidentiality_marker_preflight_reads_extractable_text(self):
        import pymupdf

        marked = self.root / "marked.pdf"
        document = pymupdf.open()
        page = document.new_page()
        page.insert_text((72, 72), "CONFIDENTIAL technical manual")
        document.save(marked)
        document.close()

        self.assertEqual(
            translate_pdf._detect_confidentiality_markers(marked),
            ("CONFIDENTIAL",),
        )
