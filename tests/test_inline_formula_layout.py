...
PROBE_FONT_PATH = next(
    (
        path
        for path in (
            Path("app/assets/GoNotoKurrent-Regular.ttf"),
...
class OrientationAndStyleTests(unittest.TestCase):
    ...
    @unittest.skipUnless(PROBE_FONT_PATH, "Unicode output font unavailable")
    def test_a_character_the_output_font_cannot_draw_is_preserved(self):
        ...
        """
        font = pymupdf.Font("probe", str(PROBE_FONT_PATH))
