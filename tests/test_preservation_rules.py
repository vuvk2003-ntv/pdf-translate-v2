...
class PreservationRuleTests(unittest.TestCase):
    ...
    def test_token_only_segments_are_filtered_before_translation(self):
        for value in (
            ...
            with self.subTest(value=value):
                self.assertFalse(needs_model_translation(value))
        self.assertTrue(needs_model_translation("D100 값을 250으로 설정하십시오."))
        self.assertTrue(needs_model_translation("Integration Test"))

    def test_common_technical_terms_may_be_preserved_when_standalone(self):
        for value in (
            "PLC",
            "HMI",
            "Servo Motor",
    ...
    def test_english_prose_and_process_labels_are_sent_for_translation(self):
        for value in (
            ...
            with self.subTest(value=value):
                self.assertTrue(needs_model_translation(value))

    def test_mixed_language_prose_is_sent_while_invariants_protect_tokens(self):
        for value in (
            "D100 값을 250으로 설정하십시오.",
            "검수 전 Manual 작성",
            "PCW Interlock 설정",
            "检查 Servo Motor 状态后启动设备。",
        ):
            with self.subTest(value=value):
                self.assertTrue(needs_model_translation(value))

    def test_one_code_line_does_not_hide_prose_in_the_same_segment(self):
        self.assertTrue(
            needs_model_translation("MOV D100 D200\nAdjust the supporting structure.")
        )

    def test_executable_code_is_preserved_as_one_usable_block(self):
        for value in (
            "MOV D100 D200",
            "DFROM_M U0 K0 D100 K2",
            "IF ready THEN MOVJ P001",
            "NWAIT",
        ):
            with self.subTest(value=value):
                self.assertFalse(needs_model_translation(value))

    def test_safe_dedup_avoids_short_context_sensitive_labels(self):
        self.assertFalse(is_safe_to_deduplicate("Manual"))
        self.assertFalse(is_safe_to_deduplicate("Qualification"))
        self.assertTrue(
            is_safe_to_deduplicate("WARNING: Disconnect power before maintenance.")
        )
    ...
    def test_table_parameter_value_and_unit_keep_separate_cells(self):
        table = SimpleNamespace(
            bbox=(0, 0, 180, 20),
            cells=[(0, 0, 100, 20), (100, 0, 140, 20), (140, 0, 180, 20)],
        )
        cells = matching_table_cells((0, 0, 180, 20), [table])
        self.assertEqual(cells, [
            (0.0, 0.0, 100.0, 20.0),
            (100.0, 0.0, 140.0, 20.0),
            (140.0, 0.0, 180.0, 20.0),
        ])
        self.assertTrue(should_translate_table_cell("Pressure"))
        self.assertFalse(should_translate_table_cell("0.5"))
        self.assertFalse(should_translate_table_cell("MPa"))
